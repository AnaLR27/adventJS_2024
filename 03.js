...
function organizeInventory(inventory) {
  ...
  // bucle que recorre el array de objetos
  for (const element of inventory) {
    // si no existe la categoria, la creamos
    if (!resultado[element.category]) {
      resultado[element.category] = {};
    }
    // si no hay juguete con ese nombre, lo guardamos
    if (!resultado[element.category][element.name]) {
      resultado[element.category][element.name] = 0;
    }

    // añadimos la cantidad de juguetes
